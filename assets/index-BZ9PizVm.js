(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))l(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const a of t.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&l(a)}).observe(document,{childList:!0,subtree:!0});function n(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function l(e){if(e.ep)return;e.ep=!0;const t=n(e);fetch(e.href,t)}})();const d=()=>{const o={imgUrl:"/pexels-restaurant-min.jpg",title:"The hottest place around",description:"A unique experience for the lovers of spicy food and those who are curious about new sensations, book a table now and don't miss out.",buttonText:"Book now"};return`
    <div class="min-h-screen flex items-center justify-center p-4">
      <div class="block md:flex p-6 justify-evenly">
          <div class="flex flex-col items-center justify-center gap-4 mb-5 lg:max-w-1/2">
            <h3 class="text-2xl md:text-3xl font-bold text-center">${o.title}</h3>
            <p class="text-xs md:text-2xl">${o.description}</p>
            <button class="px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-opacity-50">${o.buttonText}</button>
          </div>
          <img src="${o.imgUrl}" alt="restaurant image" class="md:w-90 rounded-lg">
        </div>
    </div>  
    `},u=()=>`
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col items-center justify-center md:flex-row md:justify-center flex-wrap gap-5">
            ${[{name:"Golden egg curry",imgUrl:"/dish-1.jpg",description:"soft-boiled eggs in a rich and creamy spiced curry sauce, garnished with fresh herbs.",price:12.99},{name:"Sticky sesame noodles",imgUrl:"/dish-2.jpg",description:"savory noodles topped with crispy sesame-glazed meatballs and scallions.",price:14.49},{name:"Grilled fusion platter",imgUrl:"/dish-3.jpg",description:"grilled fish and chicken paired with turmeric rice and sautéed vegetables.",price:18.99},{name:"Crispy pork bowl",imgUrl:"/dish-4.jpg",description:"crispy pork belly on rice with lettuce, caramelized onions, and chili garnish.",price:13.99},{name:"Rainbow shrimp bowl",imgUrl:"/dish-5.jpg",description:"a vibrant mix of shrimp, fresh veggies, and sesame seeds over greens.",price:15.49},{name:"Salmon poke bowl",imgUrl:"/dish-6.jpg",description:"diced salmon, avocado, edamame, cucumber, and sesame seeds on a bed of rice.",price:16.99}].map(n=>`        
        <div class="w-full md:w-72 h-96 flex flex-col bg-amber-500 rounded-xl shadow overflow-hidden">
            <h2 class="font-bold text-xl md:text-2xl text-center py-3">${n.name}</h2>
            <div class="h-40 w-full flex-shrink-0">
                <img src="${n.imgUrl}" alt="${n.name}" class="w-full h-full object-cover">
            </div>
            <div class="flex-1 flex flex-col items-center justify-between p-4 overflow-y-auto">
                <p class="md:text-lg italic">${n.description}</p>
                <p>Price: <span class="font-bold">$${n.price}</span></p>
            </div>
        </div>`).join("")}
        </div>
    </div>
    ...
    <div>
        ...
        <div class="flex flex-col items-center justify-center gap-2 p-5">
            <img src="${{imgUrl:"/cook-crew.jpg"}.imgUrl}" alt="Cook crew" class="md:w-6/12 mb-5 rounded-lg">
    ...
    `,r=document.querySelector("#app"),p=document.querySelector("#links-container"),c=d(),g=u(),x=m(),b=f();p.addEventListener("click",o=>{switch(o.target.id){case"home-button":i(),r.innerHTML=c;break;case"menu-button":i(),r.innerHTML=g;break;case"booking-button":i(),r.innerHTML=x;break;case"about-button":i(),r.innerHTML=b;break}});function i(){r.innerHTML=""}r.innerHTML=c;
